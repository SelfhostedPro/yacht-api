<template v-if="app.Ports">
    <v-btn elevation="4" variant="tonal" size="small" class="ma-1" v-for="port in CleanPorts(app)" :key="port.PrivatePort"
        :color="port.hasOwnProperty('PublicPort') ? 'primary' : 'error'"> {{
