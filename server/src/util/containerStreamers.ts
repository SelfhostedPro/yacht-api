...
export class DockerStatsStreamer {
    ...
    private async attachStatsStream(docker: Docker, containerId: string): Promise<StreamPassThrough> {
        try {
            ...
            const transformStatStream = new Transform({
                ...
                transform(chunk, enc, callback) {
                    try {
                        ...
                        console.error('Error parsing JSON chunk in transformStatStream:', err);
                    }
                    callback();
                }
            });
            ...
            console.error(`Error attaching stats stream for container \${containerId}:`, err);
        }
    }
